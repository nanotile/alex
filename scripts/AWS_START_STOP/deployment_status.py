...
class DeploymentStatusChecker:
    ...
    def calculate_totals(self, statuses: List[ModuleStatus]) -> Dict:
        """Calculate total resources and costs"""
        ...
        monthly_cost = sum(s.monthly_cost for s in deployed)
        potential_savings = sum(s.monthly_cost for s in not_deployed)
...
def print_status_report(statuses: List[ModuleStatus], totals: Dict, include_health: bool = False):
    ...
    print(f"   Current Monthly Cost: ${totals['monthly_cost']:.2f}/month")
    print(f"   Current Daily Cost: ${totals['daily_cost']:.2f}/day")
    print(f"   Maximum Possible Cost: ${totals['potential_max_cost']:.2f}/month (if all deployed)")

    if totals['not_deployed_count'] > 0:
        print(f"   {Color.OKGREEN}Savings (resources not deployed): ${totals['potential_savings']:.2f}/month{Color.ENDC}")
