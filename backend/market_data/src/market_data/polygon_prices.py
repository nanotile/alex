"""
Polygon.io price fetching — moved from backend/planner/prices.py.
"""
...
def get_share_price(symbol) -> float:
    """Get share price via Polygon. Falls back to random if API unavailable."""
    if polygon_api_key:
        try:
            return get_share_price_polygon(symbol)
        except Exception as e:
            logger.warning(f"Polygon API failed for {symbol}: {e}; using random fallback")
